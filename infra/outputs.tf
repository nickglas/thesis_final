...
output "get_credentials_command" {
  ...
  value       = "az aks get-credentials --resource-group ${local.resource_group_name} --name ${azurerm_kubernetes_cluster.rq15.name} --overwrite-existing"
}

output "push_image_command" {
  description = "Example push_acr.py invocation using the provisioned ACR."
  value       = "python scripts/push_acr.py --acr ${local.acr_name} --tag rq15-v1 --build --print-digest"
}
...
output "generate_manifests_command" {
  ...
  value       = "python k8s/aks/generate_aks_manifests.py --acr-image ${local.acr_login_server}/thesis-inference:TAG --namespace rq15 --nodepool ${var.nodepool_name}"
}
