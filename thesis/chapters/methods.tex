...
This limitation is addressed in two stages: RQ1.5 re-runs the same chain family on
managed AKS, and RQ1.5b adds an explicit multi-node sensitivity stage that forces
every chain hop across a node boundary so the inter-node penalty becomes a measured,
bounded quantity.
...
A fresh paired standard baseline is collected rather than reusing older RQ2.1
data, ensuring that incremental overhead can be attributed unambiguously to the
...
untrusted host platform, but the guest operating system, application process,
PyTorch runtime, model weights, and Istio sidecar all remain inside the same VM
trust boundary. Recent work additionally shows residual attack surfaces beyond
...
The methodological scope of this study has known limitations covering
single-node deployment, CPU-only inference, the use of a single model and
workload, controlled-versus-production variability, and the bounded threat
models claimed by RQ2.1 (mTLS communication-level hardening) and RQ2.2
(SEV-SNP VM-level confidential execution). These limitations are consolidated
and discussed in \cref{sec:analysis-threats}.
