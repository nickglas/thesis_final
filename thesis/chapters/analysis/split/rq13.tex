\subsection{Stage~L3: Explanatory Synthesis}
\label{sec:analysis-rq13}

Stage~L3 asks how activation-transfer size and compute distribution together
explain the latency and boundary-crossing behaviour observed for the
decomposition choices in Stage~L1 and Stage~L2. The analytical contribution of
...
what that model does and does not answer. Stage~L1 gave the joint reading
...
conditions. Stage~L2 supplied the controlled contrast at constant
activation size that Stage~L1 lacked. The supporting internal-timing study
...
the \(0.62\%\) downstream-compute share measured in Stage~L1
(\texttt{carry\_forward.json}). Second, the wire-cost-equal contrast
in Stage~L2 moves precisely the \texttt{layer3.1} block across the
...
(\texttt{carry\_forward.json} in both frozen runs). Stage~L2 further
...
therefore reframes the Stage~L2 tie-break: it is the rule choosing
...
The synthesis rests on the union of the Stage~L1 and Stage~L2 validation
...
Stage~L1 and between \(0.0017\) and \(0.0070\) in Stage~L2,
...
The evaluative reading of Stage~L3 has three facets. First, against the
...
under Stage~L1 and Stage~L2 conditions --- every split is measurably slower
(\(+2.4\%\) to \(+4.7\%\) in Stage~L1; \(+3.34\%\) to \(+3.64\%\) in
Stage~L2). The contribution of Stage~L3 is therefore explanatory, not
...
plus \texttt{split\_after\_layer2} (reference). Stage~L3 does not
...
\paragraph{Stage~L3 outcome.}
Under the controlled local conditions of Stage~L1 and Stage~L2, the latency
...
discovered by Stage~L2. These claims are scoped to single-host,
