\subsection{Stage~L1: Coarse Boundary Selection}
\label{sec:analysis-rq11}

Stage~L1 asks which coarse ResNet-18 stage boundaries provide suitable
...
Stage~L1 results support a single coherent reading: the cost of one
...
the Stage~L1 data make clear that this structural cleanliness is a
...
overhead in \cref{tab:rq11-results}. The Stage~L1 result is therefore
...
are deliberately absent from Stage~L1~\cite{neurosurgeon,dnnsplit}.
...
Stage~L1 therefore retains \texttt{split\_after\_layer4} as evidence for
...
above, the strongest Stage~L1 candidates are the two mid-to-late
...
defensible conclusion is narrower: under Stage~L1's controlled local
...
meaningful. This region is what Stage~L2 then refines.
...
because Stage~L1 uses one host, one build, one fixed input seed, CPU-only
batch-one inference, and localhost gRPC, and cross-host or cross-seed
replication is not in scope for this stage.

\paragraph{Evaluation.}
The evaluative reading of Stage~L1 has two facets. Against the
...
\texttt{layer4} respectively (\cref{tab:rq11-results}). Stage~L1 is
...
\texttt{split\_after\_layer2} forward to Stage~L2 rather than committing
to a single boundary at this stage.

\paragraph{Stage~L1 outcome.}
Stage~L1 provides direct but scoped evidence that the most suitable
...
Suitability under Stage~L1 conditions is therefore determined by the
