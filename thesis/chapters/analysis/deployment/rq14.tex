...
subsection. First, Stage~K establishes the service-count ordering for
the predefined Kubernetes chain family: more synchronously chained
boundaries imply more end-to-end cost in this ResNet-18 family, but the
per-hop surcharge is not a constant. Second, the chain overhead
decomposes recognisably into a roughly linear platform component and a
tensor-payload-dependent component, in line with the activation/compute
mechanisms identified in Stages~L1--L3 and synthesised in
\cref{sec:analysis-rq13}~\cite{dnnsplit,deeperthings}.

\paragraph{The service-count ordering is established in Kubernetes with measurable cost.}
...
selected. The analytical reading is therefore the first deployment-level
service-count ordering for the predefined chain family: the same
activation/compute mechanisms identified in Stages~L1--L3 remain useful
for interpreting why the in-cluster increments differ, while the
monotonic chain-depth cost itself is established here, with a worst-case
overhead bounded at +14.1\% on this hardware.
...
The adjacent differences are descriptive comparisons across the predefined
family; they should not all be read as clean causal estimates of adding one
isolated boundary, because some adjacent chain conditions also change which
split points are present.

\paragraph{The cost curve is approximately linear in service count.}
Naming the functional shape rather than only describing the trend, mean
end-to-end latency is well approximated by a straight line in the number of
chained services over the measured one-to-five range. An ordinary-least-squares
fit over the five frozen means (\cref{tab:rq14_results}) has a slope of
approximately $3.0$~ms per added service and a coefficient of determination of
$R^{2}\approx0.98$. Within this measured range, the growth is therefore
\emph{bounded and close to linear} rather than super-linear, power-law, or
exponential: the curve does not bend upward as services are added. The departures from a perfectly constant per-hop
slope are exactly the activation-transfer effect above --- the smallest increment
is the final \texttt{chain\_4svc}~$\rightarrow$~\texttt{chain\_5svc} step, which
adds only the $98$~KiB \texttt{layer4} boundary --- and this is the precise sense
in which the per-hop cost is a \emph{bounded non-linearity}: the marginal
increments vary with the activation size moved by each configuration but do not
grow without bound, so the aggregate curve stays linear in this regime. The fit
is a descriptive summary of the evaluated five-point family and is not
extrapolated beyond five services.
