...
\cref{tab:rq11-config} (\cref{sec:appendix-benchmark-configs}). In summary, the
run executed five conditions --- a monolithic baseline plus splits after
\texttt{layer1}--\texttt{layer4} --- over five rounds of 200 measured iterations
each (50 warmup iterations, 5-second cooldowns, random seed~42), with gRPC over
localhost capped at a 16~MiB message size and parity enforced at a tolerance of
$1.0 \times 10^{-5}$.
...
\begin{table}[htbp]
\centering
\caption[Stage~L1 latency, dispersion, and overhead summary]{Stage~L1 summary of mean latency, dispersion, descriptive within-run per-iteration 95\% confidence interval on the mean, and overhead vs.\ monolithic. Source:
