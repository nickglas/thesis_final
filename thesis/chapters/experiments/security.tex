...
\section{RQ2.1 (Split Sensitivity) Single-Hop mTLS vs Activation Size}
...
table-of-contents level and is not silently absorbed into the primary RQ2.1
result.
\end{quote}

\section{RQ2.1b Multi-Node Sensitivity of mTLS Overhead}
\label{sec:rq21b}
...
the primary RQ2.1 run. Every paired execution passed the multi-node placement
...
primary single-node RQ2.1 result, not as a substitute for it.
...
\begin{table}[t]
\centering
\caption{RQ2.3 synthesis of evaluated security-hardening levels. mTLS overheads are taken from \cref{tab:rq21_latency}; the selective TEE overhead is taken from \cref{tab:rq22_service2_results} relative to its own paired standard-node mTLS baseline. Each cost is reported as both the mean delta and the p95 delta, because the two security mechanisms differ in how the latency cost is distributed between the centre and the tail of the distribution.}
\label{tab:rq23_security_tradeoff}
\begin{tabular}{p{2.6cm}p{3.3cm}p{2.6cm}p{2.6cm}p{2.6cm}}
\toprule
Configuration & Protection scope & Mean cost & p95 cost & Thesis interpretation \\
\midrule
Plain AKS
& Baseline AKS deployment without mesh mTLS/AuthZ or confidential-node placement.
& Baseline (reference).
& Baseline (reference).
& Fastest evaluated Azure deployment path and the reference against which mTLS and TEE deltas are computed, but the weakest protection in the evaluated security set. Suitable only when additional communication and runtime hardening are not required. \\

mTLS/AuthZ
& Service identity, encrypted service-to-service communication, and authorization policy for the selected inference path.
& chain\_2svc: +3.488 ms / +4.62\%; chain\_5svc: +8.837 ms / +10.74\%.
& chain\_2svc: +3.623 ms / +4.53\%; chain\_5svc: +8.983 ms / +10.27\%.
& Best general-purpose hardening layer when the main requirement is authenticated and encrypted inter-service communication. Mean and p95 deltas are in the same range, so the cost is centred rather than tail-concentrated. Cost grows with protected chain depth. \\
...
& Adds runtime protection for the downstream segment that receives intermediate activations~\cite{he2021_attacking_collaborative}, bounded by the SEV-SNP threat model~\cite{schluter2024_heckler}. The tail (p95) delta is roughly 1.9$\times$ the mean delta in percent terms, so tail-sensitive SLOs should plan against the p95 number. Full two-service TEE is treated as future work. \\
\bottomrule
\end{tabular}
\end{table}
