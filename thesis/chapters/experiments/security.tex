...
stress run therefore shows that the communication-hardening cost grows when
the number of protected service boundaries grows, even though the main RQ2.1
...
\midrule
\texttt{chain\_2svc} & 3.488 & 4.62  & 2.223--5.230 & 1.064 & 3.623 \\
\texttt{chain\_5svc} & 8.837 & 10.74 & 7.623--9.985 & 0.847 & 8.983 \\
\bottomrule
\end{tabular}
}
\end{table}
...
paid when runtime protection against a stronger infrastructure threat model is
...
\midrule
\texttt{standard}     & \texttt{Standard\_D8as\_v5}  & 1000 & 58.492 & 58.283 & 60.255 \\
\texttt{confidential} & \texttt{Standard\_DC8as\_v5} & 1000 & 60.346 & 59.756 & 63.856 \\
\midrule
\textbf{Delta} & --- & --- & \textbf{+1.854 / +3.17\%} & \textbf{+1.473 / +2.53\%} & \textbf{+3.601 / +5.98\%} \\
\bottomrule
\end{tabular}
}
\end{table}
...
latency increased by 1.473~ms, and p95 latency increased by 3.601~ms.
Sequential throughput, computed from the mean latency, decreased from
17.097~req/s to 16.571~req/s, a reduction of approximately 3.08\%.
...
service-account-based policy. The \texttt{chain\_5svc} stress case shows that
this cost grows with the number of protected boundaries, reaching 8.837~ms, or
10.74\%, when the full five-service chain is protected.
