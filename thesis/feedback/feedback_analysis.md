...
**⚠️ Correction (2026-06-01, during F11):** the Phase 6 "zero stray" verification **missed `thesis/figures/tikz/results/*.tex`** — these result-figure sources still carried old IDs, including **visible** ones (the `rq15b_multinode_delta` legend "Single-node AKS (RQ1.5)" / "Multi-node AKS (RQ1.5b)", the `rq11` generator's "RQ1.1 condition" axis label) plus comments in `rq15_transfer_overhead` and the orphan `rq13_internal_summary`. All fixed during F11 (rq11 via the regenerated generator; rq15b legend → "Stage~C" / "Stage~C alternative"; comments relabelled). A fresh full-tree `RQ\d\.\d` grep now returns **only** the intentional `appendix.tex:7` manifest reference. Lesson: the Phase-6 sweep should have grepped the figure dirs explicitly.

**Known pre-existing issue (not from this work):** one "Float too large for page by ~175pt" warning on the `rq23` security trade-off table (`analysis/security.tex`) — a tall 5-column table; candidate for the F8/F15 table pass.
...
2. ✅ **F4** — Add AI-usage disclosure (+ sample prompt in annex). — confirm scope via **C3**. — **DONE 2026-06-01. C3 handled as the safe superset (narrative disclosure + named tools + sample prompts in annex); model families named so it is trivially trimmable if the program (Martin) wants less. The disclosure asserts every reference was verified against its primary source; that assertion is now substantiated by the completed F5 audit (item #7, done 2026-06-01).**
3. ✅ **F5** — Verify every reference is genuine and correctly cited. — **DONE 2026-06-01 (all 37 entries verified via Crossref/arXiv; one wrong DOI `managed_k8s_perf` found and fixed; details in §6 #7).**
...
7. ✅ **F6 + F7 + F22** — Add a Discussion reflection on generalisability (transformers/other NNs) and broader impact (per-layer policy, cross-org "digital embassies", inter-layer API). — **DONE 2026-06-01 (new §"Significance and Broader Implications" in Ch.6 + Significance paragraph in Conclusion Contributions; see §6 #8).**
8. ✅ **F16** — Add local-vs-cloud infrastructure comparison table. — **DONE 2026-06-01 (`tab:env-parity` in Methods; see §6 #9).**
9. ✅ **F11** — Add error bars / variability bands to latency figures — confirm scope via **C6**. — **DONE 2026-06-01 ($\pm$1 SD on the two absolute mean-latency figures; see §6 #10).**
10. ✅ **F12** — Name the latency-vs-service-count functional shape; reconcile "monotonic"/"non-linear" wording. — **DONE 2026-06-01 (approximately linear, OLS fit + R²; Abstract/§6.2/§7.1/figure reconciled; see §6 #11).**
11. ✅ **F13** — Ensure you can explain (and the text justifies) the statistical tests; prune any unused test. — **DONE 2026-06-01 (§4.5.3 rewritten with full rationale; no normality test exists to prune; viva cheat-sheet in chat; see §6 #12).**
12. ◐ **F8** — Enlarge in-figure fonts (Background diagrams first). — **PARTIAL 2026-06-01: 3 under-width Background figures bumped to full width; substantive font enlargement deferred to a visual-review pass (blind font bumps overflow fixed TikZ boxes; clean compile can't detect it). See §6 #13.**
...
| ✅ | 7 | Verify all references / citations | References (p.125+); in-text | Guard against fabricated refs (F5) | Medium | Pairs with #5 — **DONE 2026-06-01. All 37 `references.bib` entries verified online (Crossref API + arXiv): every cited key resolves (clean compile, no undefined citations) and every DOI/arXiv ID was confirmed to point to the correctly-titled paper. Found + fixed ONE wrong DOI: `managed_k8s_perf` was `10.1109/CloudCom.2019.00049` (resolved to an unrelated IoT-energy paper) → corrected to `10.1109/CloudCom.2019.00038` (Ferreira \& Sinnott, CloudCom 2019). The two future-dated 2026 arXiv refs (`pareto_split` 2601.08025, `noisy_neighbor_k8s` 2604.03145) confirmed real. Minor notes (no action needed): `split_object_detectors` is defined but uncited (won't print); `kaplan2023_hardware_vm` carries the article's descriptive subtitle that Crossref truncates; `hierarchical_partitioning`/HiDP is now accepted at DATE 2025 (cited as arXiv, still valid). `amd_sevsnp_whitepaper` + `schluter2024_heckler` are URL-only (no DOI) and are real well-known sources.** |
| ✅ | 8 | Add generalisability + impact reflection (transformers, per-layer policy, digital embassies, inter-layer API) | Discussion (Ch.6); cross-ref §7.2 | Reflect on applicability & significance (F6/F7/F22) | Medium | Independent — **DONE 2026-06-01. New `chapters/analysis/significance.tex` → §"Significance and Broader Implications" (between Evaluation and Threats), two subsections: (i) method generalisability — concrete what-changes for other CNNs and transformers (attention-block boundaries, sequence-length-dependent transfer, uniform compute → flatter landscape; plus 3 genuine extensions: variable seq-length contract, KV-cache state, secure-transformer crypto), distinct from the future-work lines per F6; (ii) significance of per-layer decomposition — per-layer policy, cross-org "digital embassies", inter-layer API parameterisation, all grounded in Stage H1/H2 + L3 results. F22 addressed: closing paragraph in Ch.6 + a `\paragraph{Significance.}` that closes §7.1 Summary \& Findings (cross-ref to `sec:analysis-significance`). **Template check (Template.md): Ch.6 Analysis is the template's home for "implications of your results", so the reflection is correctly placed; the significance paragraph was moved OUT of §7.2 Contributions (template scopes 7.2 to tangible results only) INTO §7.1, keeping Contributions a pure tangible-results list.** Confident but evidence-bounded. Clean compile (158 pp).** |
| ✅ | 9 | Add local-vs-cloud infrastructure comparison table | §4.2/§5.6.2 (Table 5.13 area) | Pre-empt "bigger cloud instance" critique (F16) | Low | Independent — **DONE 2026-06-01. New §"Execution Environments and Resource Parity" in `methods.tex` with `tab:env-parity`: local (AMD Ryzen 7 7800X3D, 8C/16T, 32 GB DDR5) vs local kind vs AKS (`Standard_D8s_v3` 8 vCPU/32 GiB; H2 uses `D8as_v5`/`DC8as_v5`). Rows: node CPU/mem, per-service CPU (1 core everywhere — the binding fairness control), per-service mem, threads, governor/turbo/nice. Fairness argument explicit (AKS node 8 vCPU is SMALLER than local 16-thread host; per-service budget identical). Cross-referenced from azure.tex Stage~C setup. Local specs from user (CPU-Z) + frozen `environment.json`; cloud from SKU specs. Clean compile (162 pp).** |
| ✅ | 10 | Add error bars / CI bands to latency figures | Figs 5.1, 5.3, 5.4, 5.6, 5.7 | Show measurement variability (F11) | Medium | C6; data already in CSVs — **DONE 2026-06-01. C6 resolved: added $\pm$1 SD error bars to the two ABSOLUTE mean-latency figures — Stage~L1 coarse split (regenerated via `scripts/render_rq11_figure.py`, now loads `std_ms`, widened axis, error bars + caps) and Stage~K Kubernetes chain (hand TikZ, rescaled axis 76–98 ms @0.28 u/ms to fit bars at original height). Used $\pm$1 SD (the supervisor's wording) not the 95% CI, which is invisible at N=1000; captions note both. Overhead/delta figures (transfer, multinode-delta, mTLS-delta) left as-is — error bars on derived/relative quantities are less meaningful; their dispersion is in tables. Clean compile (162 pp, no undefined refs).** |
| ✅ | 11 | Characterise latency curve shape; reconcile wording | Figs 5.3/5.4; §6.2.1, §6.2.5, Abstract | Name the function, not just "non-linear" (F12) | Low–Medium | Pairs with #10 — **DONE 2026-06-01. Named the shape: latency grows APPROXIMATELY LINEARLY in service count over the measured 1–5 range — descriptive OLS fit, local Stage~K slope $\approx$3.0 ms/svc $R^2\approx0.98$, AKS Stage~C slope $\approx$2.4 ms/svc $R^2\approx0.998$ — bounded, NOT super-linear/power-law/exponential; deviations = activation-size-modulated marginal steps. New "cost curve is approximately linear" paragraph in `analysis/deployment/rq14.tex`; AKS fit added to `rq15.tex`; Stage~K figure caption updated with fit; Abstract reworded ("bounded and non-linear" → "approximately linear … bounded rather than super-linear"); §7.1 conclusion aligned ("monotonically and approximately linearly"). The recurring "bounded non-linearity" term is now explicitly grounded as the per-hop marginal deviation, not the overall shape. Resolves the open curve-characterisation question (§8 #10): quantitative fit + qualitative name. Clean compile (160 pp).** |
| ✅ | 12 | Confirm/justify statistical tests; prepare viva explanation; prune unused tests | §4.5.3; §5.4.4, §6.4 notes; appendix | Defense readiness + rigor (F13) | Low–Medium | Independent — **DONE 2026-06-01. (a) Audited: thesis reports ONLY Cohen's d + Mann–Whitney U; NO normality test anywhere (so nothing to prune). (b) Rewrote §4.5.3 to fully justify each test: latency is right-skewed/non-normal → no normality assumption, no t-test, Mann–Whitney chosen as non-parametric; Cohen's d as N-independent effect magnitude; p-value down-weighted because N=1000 makes trivial diffs "significant"; decisions rest on predeclared carry-forward rules + cross-round CV; mean justified by CLT, median as robust cross-check. Pre-empts the supervisor's normality question directly. Existing §5/§6 notes already consistent. (c) Viva cheat-sheet provided to author in chat. Clean compile (162 pp).** |
| ◐ | 13 | Enlarge in-figure fonts; expand boxes | Figs 2.1–2.13 (then result figs) | Readability (F8) | Medium | Independent — **PARTIAL 2026-06-01. SAFE part done: 3 under-width Background figures (`service_boundary_rpc_cost` 0.96, `rq15b_chain5_multinode_placement` 0.95, `rq21_..._simple` 0.92) bumped to full `\textwidth` — uniform scale-up that enlarges text + uses whitespace, zero reflow/overflow risk. REMAINING (needs VISUAL review of the PDF — a clean compile does NOT detect text overflowing fixed TikZ nodes): the substantive font enlargement, esp. `\scriptsize` sub-labels and the `\scriptsize`-base `resnet18_detailed_basicblock_containers` (also the F9 page-filler). Background base fonts are `\small` with `\scriptsize` labels; bumping them blind risks text spilling out of fixed-size boxes. Plan: do it as a reviewed per-figure pass (user inspects PDF, flags worst offenders) or fold into the F9 figure pass. Clean compile (162 pp).** |
